...
const Socket = {
  ...
  listen: () => {
    ...
    Socket.io.on('connect', () => console.log('Conexão WebSocket estabelecida!'));
...
const FormHelper = {
  ...
  UI: {
    ...
    unlockButtons: () => {
      ...
      $('#btnVerificarResponsavel').removeAttr('disabled', '').html(Text.btnVerificarResponsavel);
    },
    startWaitTime: () => {
      // 15 seconds of waiting
      FormHelper.timerAlert = setTimeout(() => Snackbar.show('Ainda procurando...'), 15000);
    ...
    checkDefaultRadioButtons: () => {
      ...
      FormHelper.Fields.default_radio_schema.forEach((index) => $('input[type="radio"]').eq(index).click());
...
<button class="clear">Limpar</button>
      ...
      addCityName: () => {
        ...
        if ($('#labelCityName').length == 0) {
          $('<span id="labelCityName"></span>')
            .css({
              position: 'absolute', top: '470px', left: '8px', textTransform: 'uppercase', fontSize: '10px', fontWeight: 'bold', color: '#3d1ae6',
            })
            .insertAfter($('div[peid="CadastroIndividualForm.identificacaoUsuarioCidadao"]'));
  ...
  listenSaveAction: () => {
    ...
    if (saveButton.pressed) {
      ...
      if (saveButton.inPage === 'CI') console.log('💾 Salvou um cadastro Individual.');
      else if (saveButton.inPage === 'CD') console.log('💾 Salvou um cadastro Domiciliar.');
    }
  },

  formatCNS: (cns) => cns.replace(/^(\d{3})(\d{4})(\d{4})(\d{4})/, '$1 $2 $3 $4'),

  getSaveButton: () => new Promise((resolve) => {
  ...
  addEventSaveButton: async () => {
    if (!FormHelper.isEditingMode()) {
      ...
      $(saveButton).click(() => { FormHelper.saveButton = { pressed: true, inPage: RouteChecker.getCurrent() }; });
    } else {
  ...
  handleSearchSelection: (element) => {
    ...
    if (RouteChecker.isCD()) {
      console.log('Selecionou resultado de busca no CD');
      const id = $(element).data('cns').toString();
      const birthday = $(element).find('p').eq(3).text()
        .replace('No dia: ', '');
...
const API = {
  ...
  findByName: (name, birthday, mother) => {
    ...
    birthday = (birthday.indexOf('-') > -1) ? birthday.split('-').reverse().join('/') : birthday;
...
const fillUserInformation = (response) => {
  ...
  FormHelper.Fields.CI.labelCityName.text(response.municipio || 'SEM INFORMAÇÃO NA BASE DE DADOS');
  ...
  if (RouteChecker.isCI()) {
    ...
    if ($('#btnConsultarPorNome').length == 0) {
      $(`<button id="btnConsultarPorNome">${Text.btnConsultarPorNome}</button>`)
        ...
        .click(() => API.findByName(textInputs.name, textInputs.birthday, textInputs.mother));
    }
    ...
    if ($('#btnVerificarResponsavel').length == 0) {
      $(`<button id="btnVerificarResponsavel">${Text.btnVerificarResponsavel}</button>`)
        .addClass(' x-form-button x-form-field ')
        .css({
          position: 'absolute', top: '62px', left: '118px', width: '124px',
        })
    ...
    if ($('#btnAbrirFormPesquisa').length == 0) {
      $(`<button id="btnAbrirFormPesquisa">${Text.btnAbrirFormPesquisa}</button>`)
        ...
        .css({
          position: 'absolute', top: '64px', right: '78px', height: '22px', padding: '1px 10px',
        })
        .insertAfter($('div[peid="FamiliaFormEditableComponentFlexList.Confirmar"]'))
...
function MainInit() {
  ...
  // Dynamic buttons
  $(document).on('click', '.searchResult a', (event) => FormHelper.handleSearchSelection(event.currentTarget));
  $(document).on('click', '.search-container .clear', (event) => FormHelper.UI.clearSearchForm(event));
  $(document).on('click', '.search-container .do-search', (event) => FormHelper.UI.sendSearchForm(event));
}
